...
@Deprecated("SDFFont should be used in place of BMPFont")
class BMPFont(
    ttf: ByteBuffer,
    ...
    init {
        MemoryStack.stackPush().use { stack ->
            ...
            if (!STBTruetype.stbtt_PackBegin(packContext, bitmap, bitmapSize, bitmapSize, 0, 1, MemoryUtil.NULL)) error(
    ...
    fun render(encoder: Encoder, view: View, x: Double, y: Double, text: CharSequence, depth: Int = 0, color: Vector4fc = Vectors.ONE4f) {
        MemoryStack.stackPush().use { stack ->
            stack.withTransientVertexBuffer(text.length * 4, VERTEX_LAYOUT) { tvb ->
                stack.withTransientIndexBuffer(text.length * 6) { tib ->
                    ...
                    encoder.apply {
                        ...
                        setUniform(UNIFORM_COLOR, color.get(colorBuffer))
    ...
    companion object {
        ...
        private val UNIFORM_COLOR = Uniform.new("u_color0", Uniform.Type.VEC4)
        ...
        private val SHADER = ShaderProgram.loadFromFiles("/bmpfont/vs", "/bmpfont/fs")
    }
}